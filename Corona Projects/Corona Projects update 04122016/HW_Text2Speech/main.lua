...
title = display.newText("Reistration Form", cx, ity - 180, "Arrial", 30)
titleFname = display.newText("First name ", cx - 80, ity - 150, "Arial", 20)
txfFname = native.newTextField(cx + 65, titleFname.y , 170 , 30)
titleLname = display.newText("Last name ", cx - 80, ity - 100, "Arial", 20)
txfLname = native.newTextField(cx + 65, titleLname.y , 170 , 30)
titleGender = display.newText("Gender ", cx - 80, ity - 50, "Arial", 20)
txfGender = native.newTextField(cx + 65, titleGender.y , 170 , 30)
titlePosition = display.newText("Position ", cx - 80, ity, "Arial", 20)
txfPosition = native.newTextField(cx + 65, titlePosition.y , 170 , 30)
titleSalary = display.newText("Salary ", cx - 80, ity + 50, "Arial", 20)
txfSalary = native.newTextField(cx + 65, titleSalary.y , 170 , 30)
titleTel = display.newText("Tel ", cx - 80, ity + 100, "Arial", 20)
...
------------------------------------
local welcomemessage = 'Welcome Registration Form! Please pay attention to our completing demo.'
local items = {'txfFname', 'txfLname', 'txfGender', 'txfPosition', 'txfSalary', 'txfTel', 'regBtn'}

function homePageVisible( item )
	print('homePageVisible function')
	if(item=='txfFname') then 
		txfFname.isVisible = true; titleFname.isVisible = true
		transition.to(txfFname, {time = 300, y = txfFname.y - 50})
		transition.to(titleFname, {time = 300, y = titleFname.y-50})
	elseif(item=='txfLname') then 
		txfLname.isVisible = true; titleLname.isVisible = true
		transition.to(txfLname, {time = 300, y = txfLname.y - 50})
		transition.to(titleLname, {time = 300, y = titleLname.y-50})
	elseif(item=='txfGender') then txfGender.isVisible = true; titleGender.isVisible = true
		transition.to(txfGender, {time = 300, y = txfGender.y -50})
		transition.to(titleGender, {time = 300, y = titleGender.y-50})
	elseif(item=='txfPosition') then txfPosition.isVisible = true; titlePosition.isVisible =true
		transition.to(txfPosition, {time = 300, y = txfPosition.y - 50})
		transition.to(titlePosition, {time = 300, y = titlePosition.y-50})
	elseif(item=='txfSalary') then txfSalary.isVisible = true; titleSalary.isVisible = true
		transition.to(txfSalary, {time = 300, y = txfSalary.y - 50})
		transition.to(titleSalary, {time = 300, y = titleSalary.y-50})
	elseif(item=='txfTel') then txfTel.isVisible = true; titleTel.isVisible = true
		transition.to(txfTel, {time = 300, y = txfTel.y - 50})
		transition.to(titleTel, {time = 300, y = titleTel.y-50})
	elseif(item=='regBtn') then 
		print("ended")
		regBtn.isVisible = true 
		transition.to(regBtn, {time = 300, y = regBtn.y - 50})		
	end
end

title.isVisible = true
titleFname.isVisible = false
titleLname.isVisible = false
titleGender.isVisible = false
titlePosition.isVisible = false
titleSalary.isVisible = false
titleTel.isVisible = false

txfFname.isVisible = false
txfLname.isVisible = false
txfGender.isVisible = false
txfPosition.isVisible = false
txfSalary.isVisible = false
txfTel.isVisible = false
regBtn.isVisible = false

reqSpeech(welcomemessage, "en")

local message = {'This is your first name', 'This is your last name.', 'This is your gender.', 'This is your position.',
					'This is your salary. You can input the number only.', 'This is your telephone number. You can only input the number. Remember that, all items cannot be emplty. ',
					'Thank for your attention. Please enjoy! Thank you.'}

local i = 1
local function showItems( )	
	reqSpeech(message[i], 'en')
	homePageVisible(items[i])
	i = i + 1
end

transition.to(title, {time = 300, y = title.y - 80})

t = 5000
timer.performWithDelay(t, showItems, 1)
t = t + 2000
timer.performWithDelay(t, showItems, 1)
t = t + 2000
timer.performWithDelay(t, showItems, 1)
