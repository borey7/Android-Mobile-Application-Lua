...
local id, name, code, credit, year_sem, typename
local view, words
...
local function rowRender(event)
    ...
    if (row.index == 1) then
        rowTitle1 = display.newText(row, "ช์อรายวิชา", 0, 0, "Quark-Bold", fontSize + 15)    
    ...
    if(view==1) then rowString1 = string.format("%2d.", id[row.index]) .. "(" .. code[row.index] .. ")" .. name[row.index] --sort by year_sem
    elseif(view==2) then rowString1 = string.format("%2d.", id[row.index]) .. "(" .. typename[row.index] .. ")" .. name[row.index] --sort by category
    else
    	rowString1 = string.format("%2d.", id[row.index]) .. "(" .. code[row.index] .. ")" .. name[row.index] --sort by year_sem
    end
...
function createTableView(filter)
    ...
    if (view == 1) then
        sqlCommand = "SELECT * FROM subject " .. sqlFilter .. " ORDER BY year_sem;"
    elseif (view == 2) then
        sqlCommand = "SELECT * FROM subject INNER JOIN type ON subject.typeid = type.id " .. sqlFilter .. " ORDER BY type.typename;" -- by catagory
    else
        sqlCommand = "SELECT * FROM subject " .. sqlFilter .. " ORDER BY year_sem;"
    end
    print(sqlCommand)
